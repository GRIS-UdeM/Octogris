function [h,a, it] = fircls(n,be,mag,up,lo,PF,start_a)
...
%   $Revision: 1.11.4.7 $  $Date: 2009/05/23 08:13:28 $
...
if (lm ~= lu) || (lm ~= ll) || (lu ~= ll)
    error(generatemsgid('InvalidDimensions'),'Each of A, UP, and LO must have the same length.')
elseif length(be) ~= (lm+1)
    error(generatemsgid('InvalidDimensions'),'F must have one more element than A')
elseif be(1) ~= 0
    error(generatemsgid('InvalidFreqVec'),'F must begin with 0')
elseif be(lm+1) ~= 1
    error(generatemsgid('InvalidFreqVec'),'F must end with 1')
elseif any(diff(be)<=0)
    error(generatemsgid('InvalidFreqVec'),'The frequencies in F must be in ascending order.')
elseif any(up <= lo)
    error(generatemsgid('InvalidRange'),'Each element of UP must be greater than the corresponding element of LO.')
elseif any(lo > mag)
    error(generatemsgid('InvalidRange'),'Each entry of LO should be less than or equal to the corresponding element of A.')
elseif any(up < mag)
    error(generatemsgid('InvalidRange'),'Each entry of UP should be greater than or equal to the corresponding element of A.')
end
...
if nargin == 6
    ...
    switch PF(1)
        ...
        case 'b'    % as in 'both'
            TEXT_PF = 1; PLOT_PF = 1;
        otherwise,
            error(generatemsgid('InvalidParam'),'Unrecognized trace option. Try ''trace'', ''plots'', or ''both''.')
    end
end
...
if nargin == 7 && length(start_a) == length(a)
   %fprintf('using user supplied start_a (%d %d)\n', start_a, a);
   a = start_a;
end
...
while it < 1000
    ...
    if (uvo < -SN/10) || (lvo < -SN/10)
        ...
        %kmax = (1:length(A))';      kmin = (1:length(A))';
        if parity == 0
            n1 = length(kmax);
        ...
        %v1 = Amax > u(kmax)-SN;
        %v2 = Amin < l(kmin)+SN;
        v1 = Amax > u(kmax)-SN | Amax < l(kmax)+SN;
        v2 = Amin < l(kmin)+SN | Amin > u(kmin)-SN;
        kmax = kmax(v1); kmin = kmin(v2);
        cmax = cmax(v1); cmin = cmin(v2);
        Amax = Amax(v1); Amin = Amin(v2);

        % ----- check stopping criterion ------------
        Eup = abs(Amax-u(kmax)); Elo = abs(l(kmin)-Amin);
        E = max([Eup; Elo; 0]);
        if TEXT_PF
            fprintf(1,' it: %d Bound Violation = %15.13f  \n', it, E);
        end
        if E < BEST_E
           BEST_a = a;
           BEST_E = E;
...
a = BEST_a;
if E >= SN, fprintf(1,' final Bound error = %15.13f  \n', BEST_E), end;
